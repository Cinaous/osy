...
for horse, zebra in dataset:
    ...
    with tf.GradientTape(persistent=True) as tape:
        h2z = H2Z(horse)
        h2z2h = Z2H(h2z)
        h_loss = dis_lambda * mse(real_label, DZ(h2z)) + cycle_lambda * mse(horse, h2z2h)
        z2h = Z2H(zebra)
        z2h2z = H2Z(z2h)
        z_loss = dis_lambda * mse(real_label, DH(z2h)) + cycle_lambda * mse(zebra, z2h2z)
    ...
    with tf.GradientTape(persistent=True) as tape:
        z_loss = mse(real_label, DZ(zebra)) + mse(fake_label, DZ(h2z)) + mse(real_label, DZ(horse))
        h_loss = mse(real_label, DH(horse)) + mse(fake_label, DH(z2h)) + mse(real_label, DH(zebra))
