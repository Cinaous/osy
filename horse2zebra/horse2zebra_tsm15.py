...
from trm.conv_transformer import ConvTransformer
...
Z2H = kr.Sequential([
    ConvTransformer([kr.layers.Conv2D(12, 7, 2, 'same', activation='relu') for _ in range(2)]),
    ConvTransformer([kr.layers.Conv2D(48, 5, 2, 'same', activation='relu') for _ in range(2)]),
    ConvTransformer([kr.layers.Conv2D(192, 3, 2, 'same', activation='relu') for _ in range(2)]),
    ConvTransformer([kr.layers.Conv2DTranspose(48, 3, 2, 'same', activation='relu') for _ in range(2)]),
    ConvTransformer([kr.layers.Conv2DTranspose(12, 5, 2, 'same', activation='relu') for _ in range(2)]),
    ConvTransformer([kr.layers.Conv2DTranspose(3, 7, 2, 'same') for _ in range(2)]),
])
H2Z = kr.Sequential([
    ConvTransformer([kr.layers.Conv2D(12, 7, 2, 'same', activation='relu') for _ in range(2)]),
    ConvTransformer([kr.layers.Conv2D(48, 5, 2, 'same', activation='relu') for _ in range(2)]),
...
DH = kr.Sequential([
    ConvTransformer([kr.layers.Conv2D(12, 5, 2, 'same', activation='relu') for _ in range(2)]),
    ConvTransformer([kr.layers.Conv2D(48, 3, 2, 'same', activation='relu') for _ in range(2)]),
    ConvTransformer([kr.layers.Conv2D(2, 2, 1, 'same') for _ in range(2)])
])
DZ = kr.Sequential([
    ConvTransformer([kr.layers.Conv2D(12, 5, 2, 'same', activation='relu') for _ in range(2)]),
    ConvTransformer([kr.layers.Conv2D(48, 3, 2, 'same', activation='relu') for _ in range(2)]),
...
for horse, zebra in dataset:
    ...
    with tf.GradientTape() as tape:
        h2z = H2Z(horse)
        h2z2h = Z2H(h2z)
        loss = scc(real_label, DZ(h2z)) + cycle_lambda * mse(horse, h2z2h)
        z2h = Z2H(zebra)
        z2h2z = H2Z(z2h)
        loss += scc(real_label, DH(z2h)) + cycle_lambda * mse(zebra, z2h2z)
