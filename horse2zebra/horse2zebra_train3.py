from horse2zebra_datasets2 import Datasets
...
H_save_path = 'outputs/H.ckpt'
DH_save_path = 'outputs/DH.ckpt'
Z_save_path = 'outputs/Z.ckpt'
DZ_save_path = 'outputs/DZ.ckpt'
...
def test_show(test_horse=test_horse, test_zebra=test_zebra, delay=0, save=True):
    ...
    if save:
        cv2.imwrite('outputs/test_out_%d_%d.jpg' % (epoch, itr), img3)
    else:
...
while True:
    ...
    for horse, zebra in train_db:
        with tf.GradientTape(persistent=True) as tape:
            ...
            z2h2z_fake = DZ(z2h2z, True)
            loss5 = kr.losses.binary_crossentropy(tf.zeros_like(z2h2z_fake), z2h2z_fake)
            h2z2h_fake = DH(h2z2h, True)
            loss6 = kr.losses.binary_crossentropy(tf.zeros_like(h2z2h_fake), h2z2h_fake)

            loss_gan3 = kr.losses.binary_crossentropy(tf.ones_like(z2h2z_fake), z2h2z_fake)
            loss_gan4 = kr.losses.binary_crossentropy(tf.ones_like(h2z2h_fake), h2z2h_fake)

            loss_gan_total = loss_gan1 + loss_gan2 + loss_gan3 + loss_gan4
            ...
            loss_total = loss1 + 2 * loss2 + loss3 + 2 * loss4 + loss5 + loss6
