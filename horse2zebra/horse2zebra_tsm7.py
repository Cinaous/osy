...
Z2H = kr.Sequential([
    SwinTransformer(1, 2, 4, revert=True),
...
GC_save_path = 'outputs/GC27.ckpt'
DC_save_path = 'outputs/DC27.ckpt'
epoch_save_path = 'outputs/images27/epoch.npy'
loss_save_path = 'outputs/images27/loss.npy'
...
if path.exists(epoch_save_path):
    ...
    plt.plot(losses[::200, 0], label='g_loss')
    plt.plot(losses[::200, 1], label='d_loss')
...
for horse, zebra in dataset:
    if epoch % 200 == 0 or first:
        ...
        cv2.imwrite('outputs/images27/trainA&z_%d.jpg' % epoch, img3)
