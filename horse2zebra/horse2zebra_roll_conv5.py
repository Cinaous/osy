...
for horse, zebra in dataset:
    ...
    with tf.GradientTape() as tape:
        loss = scc(real_label, DZ(zebra)) + scc(fake_label, DZ(z_cache(h2z))) + scc(fake_label, DZ(horse))
        loss += scc(real_label, DH(horse)) + scc(fake_label, DH(h_cache(z2h))) + scc(fake_label, DH(zebra))
