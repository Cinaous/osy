...
#         self.qkv = kr.layers.Dense(units * 3)
#         self.window_size, self.units = window_size, units
#
#     def call(self, inputs, training=None, mask=None):
#         _, trainA, w, c = inputs.shape
#         assert trainA % self.window_size == 0 and w % self.window_size == 0, 'window size mismatch!'
#         hn, wn = trainA // self.window_size, w // self.window_size
...
#         x = tf.transpose(x, [0, 1, 3, 2, 4, 5])
#         x = tf.reshape(x, [-1, trainA, w, self.units])
#         return x
...
class SwinTransformerBlock(kr.Model):
    def __init__(self, units, window_size=4):
        super(SwinTransformerBlock, self).__init__()
        self.q = kr.Sequential([kr.layers.Dense(window_size ** 2),
                                kr.layers.LayerNormalization(),
                                kr.layers.Softmax()])
        self.v = kr.layers.Dense(units)
        self.window_size, self.units = window_size, units
