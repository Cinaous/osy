...
Z2H = UNet([32, 64, 128, 256, 512])
H2Z = UNet([32, 64, 128, 256, 512])
DH = kr.Sequential([
    ConvTransformer([kr.layers.Conv2D(48, 3, 2, 'same', activation='relu') for _ in range(2)]),
    ConvTransformer([kr.layers.Conv2D(96, 3, 2, 'same', activation='relu') for _ in range(2)]),
    ConvTransformer([kr.layers.Conv2D(192, 3, 2, 'same', activation='relu') for _ in range(2)]),
    ConvTransformer([kr.layers.Conv2D(2, 3, 1, 'same') for _ in range(2)])
])
DZ = kr.Sequential([
    ConvTransformer([kr.layers.Conv2D(48, 3, 2, 'same', activation='relu') for _ in range(2)]),
    ConvTransformer([kr.layers.Conv2D(96, 3, 2, 'same', activation='relu') for _ in range(2)]),
