...
from pix2pix.pix_models import SwinTransformer

model = SwinTransformer([1, 2], 48,
                        last_layer=kr.Sequential([
                            kr.layers.Flatten(),
                            kr.layers.Dense(10)
                        ]))
model_save_path = 'beauty17.ckpt'
scale = Scaler()

if __name__ == '__main__':
    (x_train, y_train), (x_test, y_test) = cifar10.load_data()
    x_train, x_test = scale.fit_transform(x_train), scale.fit_transform(x_test)
    model.compile(optimizer=kr.optimizers.Adamax(),
