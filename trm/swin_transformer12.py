...
class SwinUNet(kr.Model, ABC):
    def __init__(self, layers, output_dim, out_channel=3,
        ...
        for header, window_size in layers:
            ...
            swin = [MLP(n_dim, window_size, i % 2 != 0) for i in range(header)]
            ...
            if n != len(layers):
                ...
                self.ups.append(up)
                swin = [MLP(n_dim, layers[n][1], i % 2 != 0) for i in range(header)]
...
class SwinTransformer(kr.Model, ABC):
    def __init__(self, output_dim, layers, in_channel=3,
        ...
        for header, window_size in layers:
            base_layer = [MLP(2 ** n * output_dim, window_size, i % 2 != 0) for i in range(header)]
    ...
    def call(self, inputs, training=None, mask=None):
        ...
        for base_layer, merge_layer in zip(self.base_layers, self.merge_layers):
            ...
            xs = [mlp(x) for mlp in base_layer]
            x = sum(xs)
        _, h, w, c = x.shape
...
if __name__ == '__main__':
    ...
    model = kr.Sequential([
        ...
        SwinUNet([(1, 2), (1, 4)], 12),
        # 8, 4
        # SwinTransformer(48, [(2, 2), (2, 4)], n_classes=10),
        # kr.layers.Flatten(),
        # kr.layers.Dense(10)
    ])
    x = model(x_train[:1])
