...
class Dataset:
    ...
    def load_test_data(self, batch_size=1):
        return np.array([cv2.resize(img, (self.size, self.size)) for img in random.sample(self.trainA, batch_size)]), \
               np.array([cv2.resize(img, (self.size, self.size)) for img in random.sample(self.trainB, batch_size)])
