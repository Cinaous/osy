...
A2B = RestUNetModel((15, 3), 96, 3, last_layer=kr.layers.Conv2DTranspose(3, 3, padding='same'))
B2A = RestUNetModel((15, 3), 96, 3, last_layer=kr.layers.Conv2DTranspose(3, 3, padding='same'))
DA = RestNetModel((9, 3), 96, 3, last_layer=kr.layers.Conv2D(1, 3, padding='same'))
DB = RestNetModel((9, 3), 96, 3, last_layer=kr.layers.Conv2D(1, 3, padding='same'))

lr = 2e-4
cycleModel = CycleNet(A2B, B2A, DA, DB)
discriminatorModel = DiscriminatorNet(A2B, B2A, DA, DB)
cycle_save_path = './models/cycle17.ckpt'
discriminator_save_path = './models/discriminator17.ckpt'
...
epoch = 0
train_data_save_path = './images/horse2zebra17'
