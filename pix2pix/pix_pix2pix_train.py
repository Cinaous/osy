...
A2B = kr.Sequential([
    RestUNetPLayer((18, 3), 96, 3),
    kr.layers.Conv2DTranspose(3, 3, padding='same', activation=kr.activations.tanh)
])
B2A = copy.deepcopy(A2B)
DA = kr.Sequential([
    RestNetPLayer((9, 3), 96, 3),
    kr.layers.Conv2D(1, 3, padding='same', activation=kr.activations.sigmoid)
])
DB = copy.deepcopy(DA)
...
discriminatorModel.compile(optimizer=kr.optimizers.Adam(lr),
                           loss=kr.losses.mse)
cycle_save_path = './models/pix2pix_cycle.ckpt'
discriminator_save_path = './models/pix2pix_discriminator.ckpt'
...
scaler = Scaler()
real_label = np.ones([dataset.batch_size, 8, 8, 1])
fake_label = np.zeros_like(real_label)

epoch = 0
train_data_save_path = './images/pix2pix'
