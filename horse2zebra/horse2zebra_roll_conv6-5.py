...
from model_tree10 import Tree, TreeUNet
...
z_optimizer = kr.optimizers.Adam()
...
for horse, zebra in dataset:
    ...
    with tf.GradientTape() as tape:
        loss = mae(real_label, DZ(zebra)) + mae(fake_label, DZ(z_cache(h2z)))
        loss += mae(real_label, DH(horse)) + mae(fake_label, DH(h_cache(z2h)))
    grads = tape.gradient(loss, tape.watched_variables())
