...
from pix_models import RestModel, SelfRestNet
import tensorflow.keras as kr
import os.path as path
from pix_scaler import M3Scaler as Scaler

A2B = SelfRestNet(18, 96, 3, last_layer=kr.layers.Conv2D(3, 3, 1, 'same', activation=kr.activations.tanh))
B2A = SelfRestNet(18, 96, 3, last_layer=kr.layers.Conv2D(3, 3, 1, 'same', activation=kr.activations.tanh))
DA = RestModel([4 for _ in range(3)], 96, 3,
               last_layer=kr.layers.Conv2D(1, 3, 1, 'same', activation=kr.activations.sigmoid))
DB = RestModel([4 for _ in range(3)], 96, 3,
               last_layer=kr.layers.Conv2D(1, 3, 1, 'same', activation=kr.activations.sigmoid))
...
discriminatorModel.compile(optimizer=kr.optimizers.Adam(lr),
                           loss=kr.losses.mse)
cycle_save_path = './models/face2face4_cycle.ckpt'
discriminator_save_path = './models/face2face4_discriminator.ckpt'
...
epoch = 0
train_data_save_path = './images/face2face4'
