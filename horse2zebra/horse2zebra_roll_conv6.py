...
from models_model.model_rolling_conv5 import UNet, RollingConv, unet_layer
...
Z2H = UNet([unet_layer(48, 5, callback=cba),
            ...
            unet_layer(768, 5, callback=cbap),
            unet_layer(768 * 2, 3, callback=cbapd)])
DH = kr.Sequential([RollingConv(48, 5, callback=cba),
                    ...
                    RollingConv(768 * 2, 3, callback=cbapd),
                    kr.layers.Dense(2)])
DZ = kr.Sequential([RollingConv(48, 5, callback=cba),
                    RollingConv(96, 5, callback=cbap),
                    RollingConv(192, 5, callback=cbapd),
                    RollingConv(384, 5, callback=cba),
                    RollingConv(768, 5, callback=cbap),
...
mse, scc = kr.losses.MeanSquaredError(), kr.losses.SparseCategoricalCrossentropy(True)
...
for horse, zebra in dataset:
    if epoch % 25 == 0 or first:
        ...
        if first:
            first = False
            d_shape = DH(test_zebra, False).shape[1:-1]
            H2Z.summary()
            DH.summary()
    ...
    with tf.GradientTape() as tape:
        ...
        z2h = Z2H(zebra, True)
        z2h2z = H2Z(z2h, True)
        gd_loss = scc(real_label, DZ(h2z, False)) + scc(real_label, DH(z2h, False))
    ...
    with tf.GradientTape() as tape:
        loss = scc(real_label, DZ(zebra, True)) + scc(fake_label, DZ(z_cache(h2z), True))
        loss += scc(real_label, DH(horse, True)) + scc(fake_label, DH(h_cache(z2h), True))
