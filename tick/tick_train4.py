...
class RN3Net(kr.Model, abc.ABC):
    def __init__(self, filters=32, unit=kr.layers.GRU,
                 deep=9, activation=kr.activations.tanh, dropout=.2, last_layer=None):
        super(RN3Net, self).__init__()
        self.units = [(unit(filters, return_sequences=i != deep - 1),
                       kr.layers.Dense(filters)) for i in range(deep)]
        self.norm = [kr.layers.LayerNormalization(axis=1) for _ in range(deep)]
    ...
    def call(self, inputs, training=None, mask=None):
        ...
        for i, (unit, liner) in enumerate(self.units):
            if i % 2 == 0 and i:
                x += rx
                rx = cx
            cx = x = liner(unit(self.dropout(self.activation(self.norm[i](x))) if i else x))
            rx = rx if i else x
        return self.last_layer(self.activation(self.norm[0](x)))
...
if __name__ == '__main__':
    ...
    Net.compile(optimizer=kr.optimizers.Adamax(1e-4), loss=kr.losses.mse)
    epoch = 0
    for x_train, y_train in dataset:
        if epoch % 25 == 0:
            ...
            plt.savefig(f'fig4/{epoch}-train.jpg')
            plt.close()
        epoch += 1
