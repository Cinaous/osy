...
class StepsLearningRateDecay(kr.optimizers.schedules.LearningRateSchedule):
        ...
        self.initial_learning_rate.assign(
            tf.cond(step % 1000 != 0,
                    ...
                    tf.cond(step < 25000,
                            ...
                            false_fn=lambda: self.initial_learning_rate * .5)))
...
GC_save_path = 'outputs/GC24.ckpt'
DC_save_path = 'outputs/DC24.ckpt'
epoch_save_path = 'outputs/images24/epoch.npy'
loss_save_path = 'outputs/images24/loss.npy'
...
for horse, zebra in dataset:
    if epoch % 100 == 0 or first:
        ...
        cv2.imwrite('outputs/images24/trainA&z_%d.jpg' % epoch, img3)
    ...
    print('current epoch is %d: \n\tg is %.5f, d is %.5f, lr is %.20f' % (epoch, g_loss, d_loss, lr))
