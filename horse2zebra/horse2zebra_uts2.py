...
from horse2zebra_datasets10 import Datasets
from models_model.model_skip import UNet
from trm.conv_transformer5 import SkipModel, create_skips
import matplotlib.pyplot as plt
from cache_utils import Cache

batch_size = 1
dataset = Datasets(batch=batch_size)
H2Z = UNet([(48, 5, 2), (96, 3, 2), (192, 3, 2), (384, 3, 2)])
Z2H = UNet([(48, 5, 2), (96, 3, 2), (192, 3, 2), (384, 3, 2)])
DH = kr.Sequential([
    SkipModel(create_skips([(48, 5, 2), (96, 3, 2), (192, 3, 2), (384, 3, 2)], splits=2)),
    kr.layers.Dense(2)
])
DZ = kr.Sequential([
    SkipModel(create_skips([(48, 5, 2), (96, 3, 2), (192, 3, 2), (384, 3, 2)], splits=2)),
...
cycle_lambda, z_cache, h_cache = 2.5, Cache(), Cache()
...
for horse, zebra in dataset:
    ...
    with tf.GradientTape() as tape:
        h2z = H2Z(horse, training=True)
        h2z2h = Z2H(h2z, training=True)
        z2h = Z2H(zebra, training=True)
        z2h2z = H2Z(z2h, training=True)
        gd_loss = scc(real_label, DZ(h2z, training=False)) + scc(real_label, DH(z2h, training=False))
    ...
    with tf.GradientTape() as tape:
        loss = scc(real_label, DZ(zebra, training=True)) + scc(fake_label, DZ(z_cache(h2z), training=True))
        loss += scc(real_label, DH(horse, training=True)) + scc(fake_label, DH(h_cache(z2h), training=True))
