...
from models_model.model_models14 import DCModel, GCModel, UNet, Discriminator
...
DH = Discriminator([64, 128, 256, 512], 1)
DZ = Discriminator([64, 128, 256, 512], 1)
...
GC_save_path = 'outputs/GC31.ckpt'
DC_save_path = 'outputs/DC31.ckpt'
epoch_save_path = 'outputs/images31/epoch.npy'
loss_save_path = 'outputs/images31/loss.npy'
...
for horse, zebra in dataset:
    if epoch % 25 == 0 or first:
        ...
        cv2.imwrite('outputs/images31/trainA&z_%d.jpg' % epoch, img3)
    g_loss = GC.train_on_batch([horse, zebra], [real_label, real_label, horse, zebra, real_label, real_label])[0]
    d_loss = \
    DC.train_on_batch([horse, zebra], [fake_label, fake_label, real_label, real_label, fake_label, fake_label])[0]
    if first:
        first = False
        H2Z.summary()
        DH.summary()
