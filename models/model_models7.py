...
class GCModel(kr.Model, ABC):
    ...
    def call(self, inputs, training=None, mask=None):
        gy = self.G(tf.concat(inputs, axis=-1))
        dy = self.Dy(tf.concat((gy, inputs[0]), axis=-1))
        gx = self.F(tf.concat((inputs[1], inputs[0]), axis=-1))
        dx = self.Dx(tf.concat((gx, inputs[1]), axis=-1))
        x2y2x = self.F(tf.concat((gy, inputs[0]), axis=-1))
        y2x2y = self.G(tf.concat((gx, inputs[1]), axis=-1))
        x2x = self.F(tf.concat((inputs[0], inputs[0]), axis=-1))
        y2y = self.G(tf.concat((inputs[1], inputs[1]), axis=-1))
        return dy, dx, x2y2x, y2x2y, x2x, y2y

    @staticmethod
    def loss_cycle(y_true, y_predict):
        if y_true.shape != y_predict.shape:
            return kr.losses.sparse_categorical_crossentropy(y_true, y_predict)
        else:
            return kr.losses.mae(y_true, y_predict)
    ...
    def compile(self,
        ...
        super(GCModel, self).compile(optimizer, loss, metrics, loss_weights, weighted_metrics, run_eagerly,
...
class DCModel(kr.Model, ABC):
    ...
    def call(self, inputs, training=None, mask=None):
        ...
        gx = self.F(tf.concat((inputs[1], inputs[0]), axis=-1))
        dx = self.Dx(tf.concat((gx, inputs[1]), axis=-1))
        return dy, dx, self.Dy(tf.concat((inputs[1], inputs[0]), axis=-1)), self.Dx(tf.concat(inputs, axis=-1))
