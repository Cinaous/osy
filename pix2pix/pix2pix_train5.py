...
import cv2
from pix_scaler import FsScaler
from cache_utils import Cache


class CycleGAN:
    def __init__(self, label, dataset, models, lr=2e-4, m=25, scaler=FsScaler(), load=True):
        ...
        self.real_label = np.ones(models[-1].compute_output_shape(dataset.output_shape))
    ...
    def step(self, trainA, trainB):
        ...
        with tf.GradientTape(True) as tape:
            ...
            g_losses = [5. * mae(trainA, B2A(a2b)), 5. * mae(trainB, A2B(b2a)),
                        mse(real_label, DB(a2b)), mse(real_label, DA(b2a)),
                        2.5 * mae(trainA, B2A(trainA)), 2.5 * mae(trainB, A2B(trainB))]
