...
GC_save_path = 'outputs/GC11.ckpt'
epoch_save_path = 'outputs/images11/epoch.npy'
epoch, first = 0, True
...
for horse, zebra in dataset:
    if epoch % 2 == 0 or first:
        ...
        cv2.imwrite('outputs/images11/trainA&z_%d.jpg' % epoch, img3)
    DC.fit([horse, zebra], [fake_label, fake_label, real_label, real_label], 2)
    np.random.shuffle(horse)
    GC.fit([horse, zebra], [real_label, real_label, horse, zebra], 2)
    try:
