...
A2B = RestUNetModel((20, 5), 96, 3, last_layer=kr.layers.Conv2D(3, 1))
B2A = RestUNetModel((20, 5), 96, 3, last_layer=kr.layers.Conv2D(3, 1))
DA = RestNetModel((10, 5), 96, 3, last_layer=kr.layers.Conv2D(1, 1))
DB = RestNetModel((10, 5), 96, 3, last_layer=kr.layers.Conv2D(1, 1))

lr, label, lambdas = 2e-4, 9, [5, 5, 1, 1, 2.5, 2.5]
optimizer, mae, mse = kr.optimizers.Adam(lr), kr.losses.MeanAbsoluteError(), kr.losses.MeanSquaredError()
...
for trainA, trainB in dataset:
    ...
    with tf.GradientTape(watch_accessed_variables=False) as tape:
        ...
        a2b = A2B(trainA)
        b2a = B2A(trainB)
        losses = [mae(trainA, B2A(a2b)), mae(trainB, A2B(b2a)),
                  mse(real_label, DA(b2a)), mse(real_label, DB(a2b)),
                  mae(trainA, B2A(trainA)), mae(trainB, A2B(trainB))]
