...
GC_save_path = 'outputs/GC18.ckpt'
epoch_save_path = 'outputs/images18/epoch.npy'
epoch, first = 0, True
...
for horse, zebra in dataset:
    if epoch % 2 == 0 or first:
        ...
        cv2.imwrite('outputs/images18/trainA&z_%d.jpg' % epoch, img3)
    DC.fit([horse, zebra], [fake_label, fake_label, real_label, real_label], 8)
    np.random.shuffle(horse)
    np.random.shuffle(zebra)
    GC.fit([horse, zebra], [real_label, real_label, horse, zebra], 8)
    if first:
        first = False
        GC.summary()
    try:
