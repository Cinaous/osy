...
class Discriminator(kr.Model, ABC):
    def __init__(self, features, classifer=2, last_ksize=3, first_ksize=3, flatten=False, activation=None):
        ...
        self.last = kr.layers.Conv2D(classifer, last_ksize, activation=activation) if not flatten else kr.Sequential([
            kr.layers.Flatten(),
            kr.layers.Dense(classifer, activation=activation)
        ])
...
class UNet(kr.Model, ABC):
    def __init__(self, features, channel=3, last_ksize=3, first_ksize=3, activation=None):
        ...
        self.last = kr.layers.Conv2DTranspose(channel, last_ksize, 2, 'same', activation=activation)
...
class GCModel(kr.Model, ABC):
    ...
    def call(self, inputs, training=None, mask=None):
        ...
        x2y2x = self.F(gy)
        y2x2y = self.G(gx)
        return dy, dx, x2y2x, y2x2y, self.Dy(y2x2y), self.Dx(x2y2x)
...
class DCModel(kr.Model, ABC):
    ...
    def call(self, inputs, training=None, mask=None):
        ...
        gx = self.F(inputs[1])
        dx = self.Dx(gx)
        return dy, dx, self.Dy(inputs[1]), self.Dx(inputs[0]), self.Dy(inputs[0]), self.Dx(inputs[1])
