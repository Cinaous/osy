...
GC_save_path = 'outputs/GC23.ckpt'
DC_save_path = 'outputs/DC23.ckpt'
epoch_save_path = 'outputs/images23/epoch.npy'
loss_save_path = 'outputs/images23/loss.npy'
...
for horse, zebra in dataset:
    if epoch % 100 == 0 or first:
        ...
        cv2.imwrite('outputs/images23/trainA&z_%d.jpg' % epoch, img3)
    gch = GC.fit([horse, zebra], [real_label, real_label, horse, zebra, horse, zebra], callbacks=[lr_callback]).history
    dch = DC.fit([horse, zebra], [fake_label, fake_label, real_label, real_label], callbacks=[lr_callback]).history
    g_loss = gch['loss']
    d_loss = dch['loss']
    if first:
        first = False
