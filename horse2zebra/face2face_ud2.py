...
for horse, zebra in dataset:
    ...
    with tf.GradientTape() as tape:
        ...
        h2z2h = Z2H(h2z)
        loss += cycle_lambda * mse(horse, h2z2h)
        z2h = Z2H(zebra)
        loss += dis_lambda * mse(real_label, DH(z2h))
        z2h2z = H2Z(z2h)
        loss += cycle_lambda * mse(zebra, z2h2z)
    ...
    with tf.GradientTape() as tape:
        loss = mse(real_label, DZ(zebra)) + mse(fake_label, DZ(h2z)) + mse(real_label, DZ(horse))
        loss += mse(real_label, DH(horse)) + mse(fake_label, DH(z2h)) + mse(real_label, DH(zebra))
