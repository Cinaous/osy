...
from pix_models import RestUNetPLayer, RestNetPLayer
import tensorflow.keras as kr
import os.path as path
from pix_scaler import M3Scaler as Scaler

A2B = RestUNetPLayer((18, 3), 128, 3,
                     last_layer=kr.layers.Conv2DTranspose(3, 3, padding='same', activation=kr.activations.tanh))
B2A = RestUNetPLayer((18, 3), 128, 3,
                     last_layer=kr.layers.Conv2DTranspose(3, 3, padding='same', activation=kr.activations.tanh))
DA = RestNetPLayer((9, 3), 128, 3, last_layer=kr.layers.Conv2D(1, 3, padding='same', activation=kr.activations.sigmoid))
DB = RestNetPLayer((9, 3), 128, 3, last_layer=kr.layers.Conv2D(1, 3, padding='same', activation=kr.activations.sigmoid))
...
discriminatorModel.compile(optimizer=kr.optimizers.Adam(lr),
                           loss=kr.losses.mse)
cycle_save_path = './models/cycle7.ckpt'
discriminator_save_path = './models/discriminator7.ckpt'
...
dataset = Dataset('../horse2zebra', scale=20)
scaler = Scaler()
real_label = None
fake_label = None

epoch = 0
train_data_save_path = './images/horse2zebra7'
