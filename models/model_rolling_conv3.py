...
class RollingConv(kr.Model):
    def __init__(self, units, kernel_size, rolling=2, strides=2, padding='same',
                 conv=kr.layers.Conv2D, callback=None, concat=True):
        ...
        self.dense = kr.layers.Dense(1) if not concat else None

    def call(self, inputs, training=None, mask=None):
        c, n = inputs.shape[-1], len(self.convs)
        step = math.ceil(c / n)
        x = [tf.roll(inputs, i * step, axis=-1)[..., :step] for i in range(n)]
...
class UNet(kr.Model):
    def __init__(self, layers, channel=3):
        ...
        for i in range(n):
            ...
            down = RollingConv(units, kernel_size, rolling, strides, 'same', callback=callback, concat=True)
            self.downs.append(down)
            units, kernel_size, rolling, strides, callback = layers[-1 - i]
            if i == n - 1:
                break
            units, _, rolling, _, _ = layers[-2 - i]
            up = RollingConv(units, kernel_size, rolling, strides, 'same',
                             conv=kr.layers.Conv2DTranspose, callback=callback, concat=True)
            self.ups.append(up)
        self.last = RollingConv(channel, kernel_size, rolling, strides, 'same',
                                conv=kr.layers.Conv2DTranspose, concat=False)
...
if __name__ == '__main__':
    ...
    model = kr.Sequential([
        RollingConv(12, 7, 3, callback=callback2),
        RollingConv(24, 5, 2, callback=callback),
        RollingConv(48, 5, 2, callback=callback),
        RollingConv(96, 3, 2, callback=callback2),
        kr.layers.Flatten(),
        kr.layers.Dense(10)
    ])
