...
from pix_models import SRestNetModel, SRestUNetModel
import tensorflow.keras as kr
import os.path as path
from pix_scaler import M3Scaler as Scaler

A2B = SRestUNetModel([3 for _ in range(5)], 96, 3, last_layer=kr.layers.Conv2D(3, 1, activation='tanh'))
B2A = SRestUNetModel([3 for _ in range(5)], 96, 3, last_layer=kr.layers.Conv2D(3, 1, activation='tanh'))
DA = SRestNetModel([4 for _ in range(3)], 96, 3, last_layer=kr.layers.Conv2D(1, 1, activation='sigmoid'))
DB = SRestNetModel([4 for _ in range(3)], 96, 3, last_layer=kr.layers.Conv2D(1, 1, activation='sigmoid'))
...
discriminatorModel.compile(optimizer=kr.optimizers.Adam(lr),
                           loss=kr.losses.mse)
cycle_save_path = './models/cycle8.ckpt'
discriminator_save_path = './models/discriminator8.ckpt'
...
dataset = Dataset('../horse2zebra', 32)
scaler = Scaler()
real_label = None
fake_label = None

epoch = 0
train_data_save_path = './images/horse2zebra8'
