    ...
    def __call__(self, step):
        self.initial_learning_rate.assign(
            tf.cond(step % 500 != 0,
                    ...
                    false_fn=lambda:
                    tf.cond(step < 30000,
                            true_fn=lambda: self.initial_learning_rate * .96,
                            false_fn=lambda: self.initial_learning_rate * .8)))
...
GC_save_path = 'outputs/GC25.ckpt'
DC_save_path = 'outputs/DC25.ckpt'
epoch_save_path = 'outputs/images25/epoch.npy'
loss_save_path = 'outputs/images25/loss.npy'
...
for horse, zebra in dataset:
    if epoch % 100 == 0 or first:
        ...
        cv2.imwrite('outputs/images25/trainA&z_%d.jpg' % epoch, img3)
