...
from pix_scaler import MrScaler as Scaler
import copy

A2B = RestUNetModel((18, 3), 128, 3, last_layer=kr.Sequential([
    kr.layers.LayerNormalization(epsilon=1e-5),
    kr.layers.Activation('tanh'),
    kr.layers.Conv2DTranspose(3, 5, padding='same', activation='tanh')
]))
B2A = copy.deepcopy(A2B)
DA = RestNetModel((12, 3), 128, 3, last_layer=kr.Sequential([
    kr.layers.LayerNormalization(epsilon=1e-5),
    kr.layers.Activation('tanh'),
    kr.layers.Conv2D(1, 4, padding='same', activation='sigmoid')
]))
DB = copy.deepcopy(DA)
...
lr, label, lambdas = 2e-4, 'horse2zebra11', [2.5, 2.5, 1., 1., 1.5, 1.5]
...
dataset = Dataset('../horse2zebra', batch_size=2)
