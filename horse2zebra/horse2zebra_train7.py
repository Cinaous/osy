...
H_save_path = 'outputs/H7.ckpt'
DH_save_path = 'outputs/DH7.ckpt'
Z_save_path = 'outputs/Z7.ckpt'
DZ_save_path = 'outputs/DZ7.ckpt'
...
def test_show(test_horse=test_horse, test_zebra=test_zebra, delay=0, save=True):
    ...
    if save:
        cv2.imwrite('outputs/test7_out_%d_%d.jpg' % (epoch, itr), img3)
    else:
...
while True:
    ...
    for horse, zebra in zip(train_horse, train_zebra):
        ...
        try:
            ...
            pass
        print('Current epoch is %d, iter is %d, g loss is %f, cycle loss is %f, d loss is %f' %
              (epoch, itr, loss_gan_total, loss_cycle_total, loss_total))
        if itr % 50 == 0:
