...
Z2H = UNet([64, 128, 256, 512, 512])
H2Z = UNet([64, 128, 256, 512, 512])
DH = UNet([64, 128, 256, 512, 512], 1)
DZ = UNet([64, 128, 256, 512, 512], 1)
...
for horse, zebra in dataset:
    ...
    with tf.GradientTape(persistent=True) as tape:
        h2z = H2Z(horse)
        h_loss = dis_lambda * mse(real_label, DZ(h2z))
        h2z2h = Z2H(h2z)
        h_loss += cycle_lambda * mse(horse, h2z2h) + dis_lambda * mse(real_label, DH(h2z2h))
        z2h = Z2H(zebra)
        z_loss = dis_lambda * mse(real_label, DH(z2h))
        z2h2z = H2Z(z2h)
        z_loss += cycle_lambda * mse(zebra, z2h2z) + dis_lambda * mse(real_label, DZ(z2h2z))
    grads = tape.gradient(h_loss, H2Z.trainable_variables + Z2H.trainable_variables)
    g_optimizer.apply_gradients(zip(grads, H2Z.trainable_variables + Z2H.trainable_variables))
    grads = tape.gradient(z_loss, Z2H.trainable_variables + H2Z.trainable_variables)
    g_optimizer.apply_gradients(zip(grads, Z2H.trainable_variables + H2Z.trainable_variables))
    g_loss = h_loss.numpy() + z_loss.numpy()

    # train D
    DH.trainable, DZ.trainable, H2Z.trainable, Z2H.trainable = True, True, False, False
    with tf.GradientTape(persistent=True) as tape:
        z_loss = mse(real_label, DZ(zebra)) + mse(fake_label, DZ(h2z)) + \
                 mse(fake_label, DZ(z2h2z)) + mse(fake_label, DZ(horse))
        h_loss = mse(real_label, DH(horse)) + mse(fake_label, DH(z2h)) + \
                 mse(fake_label, DH(h2z2h)) + mse(fake_label, DH(zebra))
    grads = tape.gradient(z_loss, DZ.trainable_variables)
    d_optimizer.apply_gradients(zip(grads, DZ.trainable_variables))
    grads = tape.gradient(h_loss, DH.trainable_variables)
    d_optimizer.apply_gradients(zip(grads, DH.trainable_variables))
    d_loss = h_loss.numpy() + z_loss.numpy()
