...
def downs_initializer(size, channel, callback=None, scale=1, regularizer=None):
    ...
    while True:
        ...
        downs.append(conv)
    return downs, channels[:-1], ks


class UNet(kr.Model):
    def __init__(self, size=128, channel=3, callback=None, scale=1, regularizer=None):
        super(UNet, self).__init__()
        self.downs, channels, ks = downs_initializer(size, channel, callback, scale, regularizer)
        self.ups, n, i = [], len(ks), 0
        for units, (kernel_size, strides) in zip(channels[::-1], ks[::-1]):
            i += 1
...
class Discriminator(kr.Model):
    def __init__(self, size=128, channel=3, callback=None, scale=1, regularizer=None):
        ...
        self.downs = downs_initializer(size, channel, callback, scale, regularizer)[0]
        self.build([None, size, size, channel])
