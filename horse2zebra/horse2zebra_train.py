...
dataset = Datasets()
train_db = dataset.train_db(batch_size=16)
...
H_save_path = 'H.ckpt'
DH_save_path = 'DH.ckpt'
Z_save_path = 'Z.ckpt'
DZ_save_path = 'DZ.ckpt'
...
def test_show(delay=0, save=True):
    np.random.seed(None)
    ...
    if save:
        cv2.imwrite('outputs/test_out_%d.jpg' % epoch, img3)
    else:
...
while True:
    for horse, zebra in train_db:
        ...
        d_grads = d_tap.gradient(loss_total, DH.trainable_variables + DZ.trainable_variables)
        ...
        with tf.GradientTape() as g_tape:
            z2h = H(zebra, True)
            h2z = Z(horse, True)
            dh_fake = DH(z2h, False)
            dz_fake = DZ(h2z, False)
            ...
            z2h2z = Z(z2h, True)
            h2z2h = H(h2z, True)
            loss_cycle1 = kr.losses.mse(zebra, z2h2z)
            loss_cycle2 = kr.losses.mse(horse, h2z2h)
        ...
        g_grads = g_tape.gradient(loss_cycle_full, H.trainable_variables + Z.trainable_variables)
