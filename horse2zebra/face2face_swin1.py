...
from models_model.model_unet import UNet, Disc
from cache_utils import Cache
import matplotlib.pyplot as plt

batch_size = 1
dataset = Datasets(batch=batch_size)
callback = lambda conv, pool, ix: kr.Sequential([
    conv,
...
H2Z = UNet(down_callback=callback, up_callback=up_callback)
Z2H = UNet(down_callback=callback, up_callback=up_callback)
DH = Disc(kr.layers.Dense(2), callback=callback)
DZ = Disc(kr.layers.Dense(2), callback=callback)
z_cache, h_cache = Cache(), Cache()
...
for horse, zebra in dataset:
    ...
    H2Z.trainable, Z2H.trainable, DH.trainable, DZ.trainable = True, True, False, False
    ...
    grads = tape.gradient(g_loss, H2Z.trainable_variables + Z2H.trainable_variables)
    g_optimizer.apply_gradients(zip(grads, H2Z.trainable_variables + Z2H.trainable_variables))

    H2Z.trainable, Z2H.trainable, DH.trainable, DZ.trainable = False, False, True, True
    with tf.GradientTape() as tape:
        d_loss = scc(real_label, DZ(zebra)) + scc(fake_label, DZ(z_cache(h2z)))
        d_loss += scc(real_label, DH(horse)) + scc(fake_label, DH(h_cache(z2h)))
    grads = tape.gradient(d_loss, DZ.trainable_variables + DH.trainable_variables)
