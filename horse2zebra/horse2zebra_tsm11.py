...
from trm.swin_transformer15 import SwinUNet, SwinTransformer, BlockAttribute
...
DH = SwinTransformer([BlockAttribute(48, activation=kr.activations.relu,
                                     pooling=kr.layers.MaxPool2D(strides=2)),
                      BlockAttribute(96, activation=kr.activations.relu,
                                     norm=kr.layers.LayerNormalization(),
                                     drop=kr.layers.Dropout(.1)),
                      BlockAttribute(192, activation=kr.activations.relu,
                                     norm=kr.layers.LayerNormalization(),
                                     drop=kr.layers.Dropout(.1))], 1)
DZ = SwinTransformer([BlockAttribute(48, activation=kr.activations.relu,
                                     pooling=kr.layers.MaxPool2D(strides=2)),
                      BlockAttribute(96, activation=kr.activations.relu,
                                     norm=kr.layers.LayerNormalization(),
                                     drop=kr.layers.Dropout(.1)),
...
for horse, zebra in dataset:
    ...
    with tf.GradientTape() as tape:
        h2z = H2Z(horse)
        loss = dis_lambda * mse(real_label, DZ(h2z))
        h2z2h = Z2H(h2z)
        loss += cycle_lambda * mse(horse, h2z2h) + dis_lambda * mse(real_label, DH(h2z2h))
        z2h = Z2H(zebra)
        loss += dis_lambda * mse(real_label, DH(z2h))
        z2h2z = H2Z(z2h)
        loss += cycle_lambda * mse(zebra, z2h2z) + dis_lambda * mse(real_label, DZ(z2h2z))
    ...
    with tf.GradientTape() as tape:
        loss = mse(real_label, DZ(zebra)) + mse(fake_label, DZ(h2z)) + mse(fake_label, DZ(z2h2z))
        loss += mse(real_label, DH(horse)) + mse(fake_label, DH(z2h)) + mse(fake_label, DH(h2z2h))
        loss += mse(fake_label, DZ(horse)) + mse(fake_label, DH(zebra))
    grads = tape.gradient(loss, DH.trainable_variables + DZ.trainable_variables)
    d_optimizer.apply_gradients(zip(grads, DH.trainable_variables + DZ.trainable_variables))
    d_loss = loss.numpy()
