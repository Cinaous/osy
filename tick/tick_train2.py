...
class GrlLayer(kr.layers.Layer):
    def __init__(self, filters, return_sequences):
        super(GrlLayer, self).__init__()
        self.rnn = [rnn(filters, activation=None, return_sequences=return_sequences) for rnn in (
            kr.layers.SimpleRNN,
            kr.layers.GRU,
            kr.layers.LSTM)]
        self.norm = kr.layers.BatchNormalization()
        self.activation = kr.activations.tanh

    def call(self, inputs, *args, **kwargs):
        x = [rnn(inputs) for rnn in self.rnn]
        x = tf.concat(x, -1)
        return self.activation(self.norm(x))


class RN3Net(kr.Model, abc.ABC):
    def __init__(self, filters=32, unit=GrlLayer,
        ...
        self.norm = [kr.layers.BatchNormalization() for _ in range(deep)]
...
if __name__ == '__main__':
    ...
    for x_train, y_train in dataset:
        if epoch % 25 == 0:
            ...
            plt.savefig(f'fig2/{epoch}-train.jpg')
            plt.close()
        epoch += 1
