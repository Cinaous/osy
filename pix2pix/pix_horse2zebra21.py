...
A2B = RestUNetModel((9,  3), 48, 3, last_layer=kr.layers.Conv2DTranspose(3, 3, padding='same'))
B2A = RestUNetModel((9, 3), 48, 3, last_layer=kr.layers.Conv2DTranspose(3, 3, padding='same'))
DA = RestNetModel((6, 3), 48, 3, last_layer=kr.layers.Conv2D(1, 3, padding='same'))
DB = RestNetModel((6, 3), 48, 3, last_layer=kr.layers.Conv2D(1, 3, padding='same'))

label = 'horse2zebra21'
cycleModel = CycleNet(A2B, B2A, DA, DB, lr=1e-3)
discriminatorModel = DiscriminatorNet(A2B, B2A, DA, DB, lr=1e-3)
...
for trainA, trainB in dataset:
    if epoch % 25 == 0:
        ...
        test_a2b = A2B.predict(scaler.fit_transform(testA))
        test_a2b2a = B2A.predict(test_a2b)
        ...
        test_b2a = B2A.predict(scaler.fit_transform(testB))
        test_b2a2b = A2B.predict(test_b2a)
