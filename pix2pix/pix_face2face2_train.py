...
A2B = RestUNetPLayer((18, 3), 96, 3,
                     last_layer=kr.layers.Conv2DTranspose(3, 3, padding='same', activation=kr.activations.tanh))
B2A = RestUNetPLayer((18, 3), 96, 3,
                     last_layer=kr.layers.Conv2DTranspose(3, 3, padding='same', activation=kr.activations.tanh))
DA = RestNetPLayer((9, 3), 96, 3, last_layer=kr.layers.Conv2D(1, 3, padding='same', activation=kr.activations.sigmoid))
DB = RestNetPLayer((9, 3), 96, 3, last_layer=kr.layers.Conv2D(1, 3, padding='same', activation=kr.activations.sigmoid))
...
discriminatorModel.compile(optimizer=kr.optimizers.Adam(lr),
                           loss=kr.losses.mse)
cycle_save_path = './models/face2face2_cycle.ckpt'
discriminator_save_path = './models/face2face2_discriminator.ckpt'
...
epoch = 0
train_data_save_path = './images/face2face2'
