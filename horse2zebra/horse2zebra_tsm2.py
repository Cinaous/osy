...
Z2H = kr.Sequential([
    SwinTransformer(in_channel=3, revert=True),
...
DH = kr.Sequential([
    ...
    SwinTransformer(in_channel=3),
    SwinTransformer(in_channel=1),
    kr.layers.Flatten(),
    kr.layers.Dense(2)
])
DZ = kr.Sequential([
...
GC_save_path = 'outputs/GC19.ckpt'
epoch_save_path = 'outputs/images19/epoch.npy'
epoch, first = 0, True
...
for horse, zebra in dataset:
    if epoch % 2 == 0 or first:
        ...
        cv2.imwrite('outputs/images19/trainA&z_%d.jpg' % epoch, img3)
