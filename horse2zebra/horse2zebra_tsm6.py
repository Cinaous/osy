...
from trm.swin_transformer9 import SwinTransformer
...
Z2H = kr.Sequential([
    SwinTransformer(32, 8, 4),
    SwinTransformer(16, 4, 4, revert=True),
    kr.layers.Dense(3)
])
H2Z = kr.Sequential([
    SwinTransformer(32, 8, 4),
...
GC_save_path = 'outputs/GC26.ckpt'
DC_save_path = 'outputs/DC26.ckpt'
epoch_save_path = 'outputs/images26/epoch.npy'
loss_save_path = 'outputs/images26/loss.npy'
...
if path.exists(epoch_save_path):
    ...
    plt.plot(losses[::20, 0], label='g_loss')
    plt.plot(losses[::20, 1], label='d_loss')
...
for horse, zebra in dataset:
    if epoch % 200 == 0 or first:
        ...
        cv2.imwrite('outputs/images26/trainA&z_%d.jpg' % epoch, img3)
