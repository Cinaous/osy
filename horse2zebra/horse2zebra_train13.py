...
scc = kr.losses.SparseCategoricalCrossentropy()
...
for horse, zebra in dataset:
    with tf.GradientTape(persistent=True) as tape:
        ...
        # z2z = Z(zebra)
        # loss_same1 = mae(zebra, z2z)
        # h2h = H(horse)
        # loss_same2 = mae(horse, h2h)
        # loss_same_total = loss_same1 + loss_same2
        loss_same_total = 0

        loss_total = loss1 + loss2 + loss3 + loss4
        loss_cycle_full = loss_gan_total + LAMBDA * loss_cycle_total + .5 * LAMBDA * loss_same_total
    ...
    try:
        ...
        pass
    print('Current epoch is %d, g loss is %f, cycle loss is %f, d loss is %f, s loss is %f' %
          (epoch, loss_gan_total, loss_cycle_total, loss_total, loss_same_total))
    if epoch % 50 == 0:
