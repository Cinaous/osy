...
DH = UNet([64, 128, 256, 512, 512], 2)
DZ = UNet([64, 128, 256, 512, 512], 2)
...
for horse, zebra in dataset:
    ...
    with tf.GradientTape() as tape:
        h2z = H2Z(horse)
        h2z2h = Z2H(h2z)
        loss = scc(real_label, DZ(h2z)) + cycle_lambda * mae(horse, h2z2h) + scc(real_label, DH(h2z))
        z2h = Z2H(zebra)
        z2h2z = H2Z(z2h)
        loss += scc(real_label, DH(z2h)) + cycle_lambda * mae(zebra, z2h2z) + scc(real_label, DZ(z2h))
    ...
    with tf.GradientTape() as tape:
        loss = scc(real_label, DZ(zebra)) + scc(fake_label, DZ(h2z))
        loss += scc(real_label, DH(horse)) + scc(fake_label, DH(z2h))
