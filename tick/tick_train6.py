...
class Transformer(kr.Model, abc.ABC):
    def __init__(self):
        ...
        self.attention = [(
            ...
            kr.layers.LayerNormalization(1),
            kr.layers.LayerNormalization(1),
            kr.layers.Dense(32)
        ) for _ in range(5)]
    ...
    def build(self, input_shape):
        self.last_layer = kr.layers.Dense(input_shape[-1])

    def call(self, inputs, training=None, mask=None):
        x = inputs
        for attention, norm1, norm2, liner in self.attention:
            x = attention(x, x)
            x = self.activation(norm2(liner(self.activation(norm1(x)))))
...
if __name__ == '__main__':
    ...
    for x_train, y_train in dataset:
        if epoch % 25 == 0:
            ...
            plt.savefig(f'fig6/{epoch}-train.jpg')
            plt.close()
        epoch += 1
