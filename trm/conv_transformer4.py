...
class ConvTransformer(kr.Model):
    def __init__(self, convs, units=3, splits=2, kernal_size=3,
                 pooling=kr.layers.MaxPooling2D):
        super(ConvTransformer, self).__init__()
        self.convs, self.splits = convs, splits
        self.conv = kr.Sequential([
            kr.layers.Conv2D(units, kernal_size, padding='same'),
            pooling(2, 1, 'same')
        ])
...
def create_skips(layers, con=kr.layers.Conv2D,
                 skip=2, start_activation=2, splits=None, start_norm=2,
                 norm=kr.layers.LayerNormalization, epsilon=1e-5,
                 pooling=kr.layers.MaxPooling2D,
    ...
    for units, kernel_size, strides in layers:
        ...
        conv = con(units, kernel_size, strides, 'same', activation=activation_) if splits is None \
            else ConvTransformer([con(units, kernel_size, strides, 'same', activation=activation_)
                                  for _ in range(splits)], units, splits, kernel_size, pooling)
        if i > start_norm and norm is not None:
            conv = kr.Sequential([conv, norm(epsilon=epsilon)])
        if i % skip == 0:
            conv_ = kr.Sequential([con(units, strides_ * kernel_size, strides_ * strides, 'same'), pooling]) \
                if splits is None else \
                ConvTransformer([con(units, strides_ * kernel_size, strides_ * strides, 'same')
                                 for _ in range(splits)], units, splits, kernel_size, pooling)
...
class SkipModel(kr.Model):
    def __init__(self, skips):
        ...
        self.denses = [kr.layers.Dense(1) if hasattr(down_skip, '__len__') else None for down_skip in skips]

    def call(self, inputs, training=None, mask=None):
        x, i, n = inputs, 0, len(self.down_skips)
        for down_skip, dense in zip(self.down_skips, self.denses):
...
if __name__ == '__main__':
    ...
    model = kr.Sequential([
        SkipModel(create_skips([(12, 5, 2), (48, 3, 2), (96, 3, 2)], splits=2)),
        SkipModel(create_skips([(48, 3, 2), (12, 3, 2), (3, 5, 2)], con=kr.layers.Conv2DTranspose, splits=2))
    ])
