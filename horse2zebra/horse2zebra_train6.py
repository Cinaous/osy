...
g_optimizer = kr.optimizers.Adam(2e-8, beta_1=.5)

H_save_path = 'outputs/H6.ckpt'
DH_save_path = 'outputs/DH6.ckpt'
Z_save_path = 'outputs/Z6.ckpt'
DZ_save_path = 'outputs/DZ6.ckpt'
...
def test_show(test_horse=test_horse, test_zebra=test_zebra, delay=0, save=True):
    ...
    if save:
        cv2.imwrite('outputs/test6_out_%d_%d.jpg' % (epoch, itr), img3)
    else:
...
while True:
    ...
    for horse, zebra in zip(train_horse, train_zebra):
        horse, zebra = np.array([horse]), np.array([zebra])
